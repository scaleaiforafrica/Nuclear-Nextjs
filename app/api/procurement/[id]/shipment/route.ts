...
/**
 * GET /api/procurement/[id]/shipment
...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: 'Procurement request ID is required',
        },
        { status: 400 }
      );
    }

    // Fetch shipment by procurement_request_id
        ...
        return NextResponse.json(
          {
            ...
            error: `No shipment found for procurement request ID: ${id}`,
          },
          { status: 404 }
        );
      }

      console.error('Error fetching shipment by procurement ID:', error);
      return NextResponse.json(
        {
          success: false,
    ...
    console.error('Unexpected error in GET /api/procurement/[id]/shipment:', error);
    return NextResponse.json(
      {
        success: false,
